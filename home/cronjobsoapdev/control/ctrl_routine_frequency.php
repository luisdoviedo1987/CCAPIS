...
$apiStartDate = "2021-09-06";
$apiEndDate = "2021-09-06"; 

echo  "<h1> data_neoapi_tickets_RRSS_mail_25</h1>"; 
echo $apiStartDate;// $resultCronJob["vStartDate"]; 
echo "<br>";
echo $apiEndDate ; // $resultCronJob["vEndDate"];


$response = $model->consumirServicio("https://canales.medismart.net/NEOAPI/WEBSERVICE.ASMX?wsdl", array("idTask"=> "25","param1"=> $apiStartDate, "param2"=> $apiEndDate));
...
if (strpos($response["ExecuteTask02Result"], '{"Table":[{') !== false) {
    ...
    if (is_array($dataNeo)){
        foreach($dataNeo as $key => $row ){
            ...
            $insert =  $model->insertSQLTable("data_neoapi_tickets_RRSS_mail_25",$row , $dataNeo[0], "1") ;
    ...
    if (is_array($dataNeo)){   
        $insert =  $model->insertSQLTable("data_neoapi_tickets_RRSS_mail_25",$dataNeo , $dataNeo, "1") ;
