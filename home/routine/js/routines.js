...
var page = {
        ...
        $("#wizard").on("leaveStep", function(e, anchorObject, stepIndex, stepDirection) {
            if(stepIndex == 0){
                if($("#clipboard-default").val() ==""){
                    global.myToast(4, "Error", 'Por favor indique la dirección ULR a consumir', 'toast-top-right', 'toast-top-right');
                    return false;
                }
            }
         ...
         $("#wizard").on("showStep", function(e, anchorObject, stepIndex, stepDirection, stepPosition) {
            if(stepIndex == 1){
                global.myToast(3, "Información", 'Por favor pruebe el servicio web', 'toast-top-right', 'toast-top-right');
            }
        });
    ...
    loadTypes: function () {
        ...
        response.then(function (data) {
            ...
            if (data.error) {
                ...
                $("#typeBodySelect").empty().append(global.selectOptions('id_body_type', 'name', data, null, 'is_raw'));
            }
    ...
    generateBlock: function (count, custom) {
        return '<div class="row '+custom+'">' +
            ...
            '<div class="col-2">' +
                '<div class="checkbox">' +
                    '<input id="check' + count + '" type="checkbox" class="checkChange" value="1">' +
                    '<label for="check' + count + '"> Incrementa </label>' +
                '</div> ' +
            '</div> ' +
    ...
    getDataFromTab(idtab) {
        var result = []; 
        $(idtab).find('.container').children().not('.divRawForm').each(function (i, element) {
            var row = {};
...
$(document).ready(function () {
    ...
    $("#btn-create-routine").bind( "click", function() {
        ...
        if($("#day").val() == 0 && $("#hour").val() == 0 && $("#minutes").val() == 0) {
            global.myToast(4, "Error", 'Por favor ingrese la recurrencia', 'toast-top-right', 'toast-top-right');
            return; 
        }
       
        if(!validService){
            global.myToast(4, "Error", 'Por favor pruebe el servicio', 'toast-top-right', 'toast-top-right');
            return; 
        }

        if(!readed){
            global.myToast(4, "Error", 'Por favor ingrese el nivel', 'toast-top-right', 'toast-top-right');
            return; 
        }
